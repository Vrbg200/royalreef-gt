    ...
    <>
      <nav style={{
        ...
        <Link href="/" style={{ textDecoration: 'none' }}>
          <span style={{ fontSize: '17px', fontWeight: 500, letterSpacing: '0.08em', color: '#C9A84C' }}>
        ...
        <div style={{ display: 'flex', gap: '24px', alignItems: 'center' }}>
          ...
          {user ? (
            <Link href="/profile" style={{ fontSize: '13px', color: '#C9A84C', textDecoration: 'none' }}>
              Mi cuenta
            </Link>
          ) : (
      ...
      {cartOpen && (
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
            <span style={{ fontSize: '14px', fontWeight: 500, color: '#F0EDE6' }}>Carrito ({count})</span>
            <button onClick={() => setCartOpen(false)} style={{ background: 'transparent', border: 'none', color: '#8A8680', cursor: 'pointer', fontSize: '18px' }}>✕</button>
          </div>
              ...
              <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {items.map(item => (
                  <div key={item.id} style={{
                    background: '#1E1E1E', border: '0.5px solid rgba(201,168,76,0.18)',
                    borderRadius: '6px', padding: '10px 12px',
                    display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start',
                  }}>
                    <div>
                      <div style={{ fontSize: '12px', fontWeight: 500, color: '#F0EDE6', marginBottom: '2px' }}>{item.name}</div>
                      <div style={{ fontSize: '10px', color: '#8A8680' }}>{item.code} · {item.size}</div>
                      <div style={{ fontSize: '13px', fontWeight: 500, color: '#C9A84C', marginTop: '4px' }}>Q {item.price.toLocaleString()}</div>
                    </div>
                    <button onClick={() => removeItem(item.id)} style={{ background: 'transparent', border: 'none', color: '#8A8680', cursor: 'pointer', fontSize: '14px' }}>✕</button>
                  </div>
                ))}
              </div>

              <div style={{ borderTop: '0.5px solid rgba(201,168,76,0.18)', paddingTop: '16px', marginTop: '16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '14px' }}>
                  <span style={{ fontSize: '13px', color: '#8A8680' }}>Total</span>
                  <span style={{ fontSize: '16px', fontWeight: 500, color: '#C9A84C' }}>Q {total.toLocaleString()}</span>
                </div>
